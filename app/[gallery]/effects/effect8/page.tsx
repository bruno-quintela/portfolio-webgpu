...
import TEXTUREMAP from '@/assets/raw-7.png';
import NORMALMAP from '@/assets/normal.png';
import DEPTHMAP from '@/assets/depth-7.png';
...
const Scene = () => {
  ...
  const { material, uniforms } = useMemo(() => {
    ...
    const strength = 0.02;
    const maxRGBShift = float(0.05); // Increased from 0.008
    ...
    // Calculate parallax effect strength
    const parallaxStrength = initialDepth.r.mul(uPointer.length()).mul(strength);
    
    // Create base UV with parallax
    ...
    // Dynamic RGB Shift based on parallax strength
    const rgbShiftAmount = maxRGBShift.mul(parallaxStrength);
    const redUv = displacedUv.add(vec2(rgbShiftAmount, 0));
    const blueUv = displacedUv.sub(vec2(rgbShiftAmount, 0));
