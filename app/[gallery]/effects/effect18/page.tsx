...
import { ComputePetalParticles } from '@/components/petalParticles';
...
import SPRITETEXTURE from "@/assets/petal.png";
...
const Scene = () => {
  ...
  const { material, material2, material3, material4, uniforms } = useMemo(() => {
    ...
    finalColor4 = vec4(textureSample4.rgb, textureSample4.r.pow(2));
  ...
  useFrame((state, delta) => {
    ...
    if (meshRef.current) {
      ...
      const rotY = THREE.MathUtils.lerp(
       ...
       if (particleRef.current) {
        particleRef.current.rotation.x = rotX*1.5;
        particleRef.current.rotation.y = rotY*1.5;
       }
    }
    ...
    if (meshRef.current) {
      ...
      if (particleRef.current) {
        //particleRef.current.position.x = translateX*.25;
        particleRef.current.position.y = -scrollOffset/2;
      }
    }
    ...
    if (meshRef2.current) {
      ...
      if (meshRef4.current) {
        meshRef4.current.position.x = translateX*.25;
        meshRef4.current.position.y = translateY*.25;
      }
      if (meshRef3.current) {
        meshRef3.current.position.x = translateX*1.2;
        meshRef3.current.position.y = translateY*1.2;
      }
  ...
  const particleTexture = useTexture(SPRITETEXTURE.src);

  return (
    <>
      <ambientLight intensity={1} color="#ffffff" />
     
      <ComputePetalParticles ref={particleRef} texture={particleTexture} />
      {/*front plane */}
      <mesh
        ref={meshRef4}
        scale={[1.*w, 1.*h, 1]}
...
const Html = () => {
  ...
  return (
    <div>
      <div
        className="h-svh fixed z-20 bg-gray-500 pointer-events-none w-full flex justify-center items-center"
        data-loader
      >
