...
import TEXTUREMAP from '@/assets/raw-8.png';
import NORMALMAP from '@/assets/normal.png';
import DEPTHMAP from '@/assets/depth-8.png';
...
const Scene = () => {
  ...
  const { material, uniforms } = useMemo(() => {
    ...
    const strength = 0.025; // Increased for more noticeable effect
    const maxRGBShift = float(0.25);
    ...
    const tiltDirection = vec2(
        abs(uPointer.x).div(pointerLength.add(0.0001)),  // Horizontal weight
        abs(uPointer.y).div(pointerLength.add(0.0001))   // Vertical weight
    );
    ...
    const blurStrength = parallaxStrength.mul(0.1); // Adjust blur intensity
    ...
    for (let i = 0; i < maxBlurSamples; i++) {
      ...
      const rgbOffset = vec2(
          ...
          rgbShiftAmount.mul(tiltDirection.y).mul(2.0)
      );

      // Sample each color channel with directional offset
