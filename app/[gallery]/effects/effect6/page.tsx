...
const Scene = () => {
  ...
  const { material, uniforms } = useMemo(() => {
    ...
    const elevation = tDepthMap.r.mul(0.03); // Adjust multiplier to control displacement amount
    const positionLocal = attribute('position');
    const position = positionLocal.add(vec3(0, elevation, 0));

    const material = new THREE.MeshStandardNodeMaterial({
      colorNode: final,
      //normalNode: texture(normalMap, uv()),
      //metalnessNode: texture(depthMap, uv()),
      //roughnessNode: texture(depthMap, uv()),
      roughness: 1,
      metalness: 1,
    });
  ...
  useFrame((state) => {
    ...
    pointerRef.current.lerp(state.pointer, 0.041);
    ...
    if (lightRef.current && pointLightRef.current) {
      ...
      pointLightRef.current.position.z = 2;
    }
  });
