...
function GridTiles({ tiles, groups, isSubgrid = false, gridRefs, subgridRefs }) {
...
function Scene() {
  ...
  useEffect(() => {
    ...
    const onTouchMove = (e: TouchEvent | MouseEvent) => {
      ...
      if (isInteractingWithSubgrid()) {
        // Subgrid: only horizontal scroll, but not if a subgrid tile is selected
        const isSubgridTileSelected = clickedMeshRef.current && isSubgridMesh(clickedMeshRef.current);
        if (isSubgridTileSelected) {
          // Do not allow horizontal scroll
          scrollRef.current.target = {
            x: scrollRef.current.position.x,
            y: scrollRef.current.position.y
          };
        } else {
          const distanceX = (startPosRef.current.x - clientX) * scrollRef.current.scale;
          scrollRef.current.target = {
            x: scrollRef.current.position.x - distanceX,
            y: scrollRef.current.position.y // Keep Y fixed for subgrid
          };
        }
      } else {
        ...
        scrollRef.current.target = {
          x: scrollRef.current.position.x, // Keep X fixed for main grid
    ...
    const onClick = (event: MouseEvent) => {
      ...
      if (intersects.length > 0) {
        ...
        if (!isSubgrid && clickedMesh.userData && typeof clickedMesh.userData.col === 'number' && clickedMesh.userData.col !== Math.floor(GRID_TILES_X / 2)) {
        ...
        if (clickedMeshRef.current === clickedMesh) {
          if (isSubgrid) {
            ...
            allMeshes.forEach((mesh) => {
              ...
              if (isSubgrid) {
                meshPos.add(subgridGroupRefs.current[meshGroupIndex].ref.position);
              } else {
          ...
          if (groupIndex !== -1) {
            ...
            clickedPos.add(subgridGroupRefs.current[subgridGroupIndex].ref.position);
          }
          ...
          if (groupIndex !== -1) {
            ...
            clickedMeshRef.current.targetScale = { x: SELECTED_TILE_WIDTH_RATIO, y: 1, z: 1 }; // When opening
            ...
            targetCenterRef.current = {
              x: -(i * TILE_SPACE_X - TILE_SPACE_X) - groupCol * GRID_SIZE_X - groupOffset.x,
            ...
            clickedMeshRef.current.targetScale = { x: SELECTED_TILE_WIDTH_RATIO, y: .9, z: 1 }; // When opening
            ...
            const groupOffset = subgridGroupRefs.current[subgridGroupIndex].offset;
            
            // Calculate the target scroll position to center this subgrid cell
            // Only adjust X position since subgrid is horizontal
            targetCenterRef.current = {
              x: -(i * TILE_SPACE_X - TILE_SPACE_X) - SUBGRID_TILE_GROUPS[subgridGroupIndex].pos[0] - groupOffset.x,
  ...
  const checkClicked = () => {
    ...
    if (mainGridSelectionRef.current) {
      ...
      mainGridMeshes.forEach((mesh) => {
        const shouldAnimate = mesh === mainGridSelectionRef.current || 
      ...
      subgridMeshes.forEach((mesh) => {
        ...
        const shouldAnimate = mesh.animationStartTime !== undefined && 
          currentTime >= mesh.animationStartTime + (mesh.animationDelay || 0) + SUBGRID_ANIMATION_DELAY;
        ...
        if (shouldAnimate) {
          ...
          if (mesh === clickedMeshRef.current) {
            ...
            if (mesh.userData.originalScale === undefined) {
              mesh.userData.originalScale = { x: SELECTED_TILE_WIDTH_RATIO, y: 1, z: 1 };
            }
    ...
    if (clickedMeshRef.current && isSubgridMesh(clickedMeshRef.current)) {
      ...
      allSubgridTiles.forEach((mesh) => {
        if (mesh !== selectedTile) {
          ...
          const maxOffset = TILE_SPACE_X * .75;
          const falloff = .95;//Math.exp(-distance * 0.5);
          ...
          if (!isDraggingRef.current) {
            ...
            if (mesh.userData.targetX === undefined) {
              const originalX = (mesh.userData.col - Math.floor(GRID_TILES_X / 2)) * TILE_SPACE_X;
            ...
            mesh.position.x = lerp(mesh.position.x, mesh.userData.targetX, isDraggingRef.current ? 0.02 : 0.05);
            
            // Scale down non-selected subgrid tiles to a smaller width
            mesh.targetScale = { x: 1, y: .75, z: 1 };
          }
        }
      });
    } else {
      ...
      subgridMeshes.forEach(mesh => {
        ...
        if (!isDraggingRef.current) {
          ...
          if (mesh.userData.originalX !== undefined) {
            mesh.position.x = lerp(mesh.position.x, mesh.userData.originalX, 0.05);
          } else {
  ...
  const setPositions = () => {
    ...
    if (!isZoomingRef.current) {
      gridGroupRefs.current.forEach(({ ref, offset }, i) => {
        ...
        const posX = pos[0] + offset.x; // Main grid: no scrollX
        const posY = pos[1] + scrollY + offset.y; // Main grid: only scrollY
    ...
    // Subgrid: only scroll horizontally
    subgridGroupRefs.current.forEach(({ ref, offset }, i) => {
      const pos = SUBGRID_TILE_GROUPS[i].pos;
      const dir = directionRef.current;
      const posX = pos[0] + scrollX + offset.x; // Subgrid: only scrollX
...
function ReflectiveFloor({ clickedMeshRef, subgridGroupRefs, mainGridSelectionRef }) {
  ...
  useFrame(() => {
    ...
    const hasDelayPassed = subgridModeStartTimeRef.current !== null && 
    ...
    const isSubgridTileSelected = clickedMeshRef.current !== null && isSubgridMesh(clickedMeshRef.current);
    const targetY = isSubgridTileSelected ? -1.65 : -1.25;
    ...
    if (materialRef.current.opacity < (isSubgridMode ? .25:.75)) {
...
const updateMesh = (
  ...
  if (isSelected || isMainGridSelected) {
    // Selected state
