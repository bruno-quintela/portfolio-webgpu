...
function Scene() {
  ...
  // Add eased scroll offsets for smooth animation
  const easedRowScrollOffsetsRef = useRef(Array(GRID_TILES_Y).fill(0));
  ...
  useEffect(() => {
    ...
    const onClick = (event: MouseEvent) => {
      ...
      if (intersects.length > 0) {
        ...
        // --- Center the row on the clicked tile (accounting for infinite wrapping) ---
        if (
          clickedMesh.userData &&
          typeof clickedMesh.userData.row === 'number' &&
          typeof clickedMesh.userData.col === 'number'
        ) {
          const row = clickedMesh.userData.row;
          const initialX = clickedMesh.userData.initialX;
          
          // Find which group this mesh belongs to
          let groupIndex = -1;
          gridGroupRefs.current.forEach(({ ref }, idx) => {
            if (clickedMesh.parent === ref) groupIndex = idx;
          });
          const groupOffset = groupIndex !== -1 ? gridGroupRefs.current[groupIndex].offset.x : 0;
          
          // Find the center column's initial X position
          const centerCol = Math.floor(GRID_TILES_X / 2);
          const centerTile = GRID_TILES.find(
            t => t.row === row && t.col === centerCol
          );
          const centerTileX = centerTile ? centerTile.pos[0] : 0;
          
          // Calculate the current effective position of the clicked tile
          // (initialX + current row offset + group offset)
          const currentRowOffset = rowScrollOffsetsRef.current[row];
          const currentEffectiveX = initialX + currentRowOffset + groupOffset;
          
          // Calculate the shortest path to center the clicked tile
          const rowWidth = GRID_SIZE_X;
          let targetOffset = centerTileX - initialX - groupOffset;
          
          // Find the shortest path considering wrapping
          const currentWrapped = ((currentEffectiveX % rowWidth) + rowWidth) % rowWidth;
          const targetWrapped = ((centerTileX % rowWidth) + rowWidth) % rowWidth;
          
          // Calculate the minimal offset needed
          let delta = targetWrapped - currentWrapped;
          if (delta > rowWidth / 2) delta -= rowWidth;
          if (delta < -rowWidth / 2) delta += rowWidth;
          
          // Apply the minimal offset to the current row offset
          rowScrollOffsetsRef.current[row] = currentRowOffset + delta;
        }

        // Prevent selecting main grid tiles that are not in the center column (for subgrid opening only)
        ...
        if (clickedMeshRef.current === clickedMesh) {
          ...
          if (groupIndex !== -1) {
            ...
            // --- Sync eased offset to raw offset for the new row ---
            const row = clickedMesh.userData.row;
            easedRowScrollOffsetsRef.current[row] = rowScrollOffsetsRef.current[row];
        ...
        ref.children.forEach((child: ExtendedMesh) => {
          if (child.userData && typeof child.userData.row === 'number' && typeof child.userData.col === 'number') {
            ...
            if (selectedRowRef.current !== null && row === selectedRowRef.current) {
              // Get the eased row scroll offset for smooth animation
              let rowOffset = easedRowScrollOffsetsRef.current[row];
              
              // Implement infinite wrapping at the row level (not individual tiles)
              const rowWidth = GRID_SIZE_X;
              
              // If row has scrolled too far right, wrap it to the left
              while (rowOffset > rowWidth) {
                rowOffset -= rowWidth;
              }
              // If row has scrolled too far left, wrap it to the right
              while (rowOffset < -rowWidth) {
                rowOffset += rowWidth;
              }
              
              // Apply the wrapped offset to the tile position
              targetChildX = initialXPosition + rowOffset;
              
              // For selected row, apply position directly without lerp to prevent snapping
              child.position.x = targetChildX;
            } else {
              // For non-selected rows, use lerp for smooth transitions
  ...
  useFrame((state) => {
    ...
    // Ease the row scroll offsets for smooth animation
    if (selectedRowRef.current !== null) {
      const selectedRow = selectedRowRef.current;
      easedRowScrollOffsetsRef.current[selectedRow] = lerp(
        easedRowScrollOffsetsRef.current[selectedRow],
        rowScrollOffsetsRef.current[selectedRow],
        0.05
      );
    }
